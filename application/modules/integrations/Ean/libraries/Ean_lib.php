...
class Ean_lib {
	...
	function __construct($_apiKey = "n2cxz49a5vd9u9verw3afva7" ,$_local = "en",$_currency = "USD"){
        ...
        $lang_set = $this->ci->session->userdata('set_lang');
...
//get popular hotels home page
  function getHomePagePopularHotels() {
      ...
      $response->hotels = $this->getShortResultsObject($result, $resultarray, $arrayInfo['checkIn'], $arrayInfo['checkOut']);
...
//get featured hotels home page
  ...
  function getRelatedHotels($city,$ccheckin,$ccheckout) {
    if(empty($ccheckin)){
    $checkin = date("m/d/Y", strtotime("+1 days"));
  }else{
    $checkin = $ccheckin;
  ...
  $checkout = $ccheckout;
     ...
     $response->hotels = $this->getShortResultsObject($result, $resultarray,$checkin,$checkout);
	...
	function HotelLists($arrayInfo){
           ...
           $str= $this->apiurl.'list?minorRev='.$this->rev.'&cid='.$this->cid.'&sig='.$this->sig.
                    '&apiKey='.$this->apiKey.'&customerSessionId&customerUserAgent='.$this->customerUserAgent.'&locale='.$this->local.'&customerIpAddress='.$this->customerIpAddress.
                    '&currencyCode='.$this->currency.
                    '&destinationString='.$city.'&arrivalDate='.$checkIn.'&departureDate='.$checkOut.'&'.$rooms.'&numberOfResults='.$numberOfResult.
...
//         */
 ...
 /*
        ...
        function getShortResultsObject($result, $resultarray, $checkin,$checkout){
            $this->ci->load->helper('text');
            $resultData = array();
            foreach($resultarray as $res){

            $thumbnail = str_replace("_t","_b",$res['thumbNailUrl']);
            $slug = $this->baseUrl."hotel"."/".$res['hotelId']."/".$result['HotelListResponse']['customerSessionId'].'/'.str_replace("/","-",$checkin)."/".str_replace("/","-",$checkout);
      ...
      function totalStay($ckin, $chkout){
        ...
        if(empty($ckin)){
        $ckin = date("m/d/Y", strtotime("+1 days"));
      }else{
        $ckin = $ckin;
      }

      if(empty($chkout)){
      $chkout = date("m/d/Y", strtotime("+2 days"));
    }else{
      $chkout = $chkout;
    }
      $start = strtotime($ckin);
      $end = strtotime($chkout);
      $this->checkinStr = date("F j, Y",$start);
      $this->checkoutStr = date("F j, Y",$end);
