...
// $this->session->sess_destroy();
		...
		public function index() {
				...
				$this->data['totalResults'] = $result->totalCounts;

				$this->data['eancheckin'] = date("m/d/Y", strtotime("+1 days"));
				$this->data['eancheckout'] = date("m/d/Y", strtotime("+2 days"));

				$this->data['checkinStr'] = $this->Ean_lib->checkinStr;
				$this->data['checkoutStr'] = $this->Ean_lib->checkoutStr;
				...
				// $cookie = array(
        // 'name'   => 'page2',
        // 'value'  => json_encode(array('cachekey' => $result->cacheKey, 'cacheloc' => $result->cacheLocation)),
        // 'expire' => time()+86400,
        // 'domain' => '',
        // 'path'   => '/',
        // 'prefix' => '',
		...
		$this->data['roomAmenities'] = $this->Ean_model->getRoomAmenities($this->data['lang_set']);
			...
			$this->data['cacheLocation'] = $result->cacheLocation;
			$this->data['module'] = $result->hotels;
			//$this->theme->partial('integrations/hotels/ean/morehotels', $this->data);

			// $paginationinfo = array('base' => 'hotels/listings', 'totalrows' => $result->totalCounts, 'perpage' => 10,'urisegment' => $totalSegments);
			// $this->data['info'] = $paginationinfo;
			//
			$this->setMetaData($this->settings[0]->header_title, $this->settings[0]->meta_description,$this->settings[0]->meta_keywords);
			$this->data['langurl'] = $this->data['baseUrl']."{langid}/";
			$this->theme->view('listing', $this->data, $this);
		...
		function search($offset = null) {
			...
			$this->data['roomAmenities'] = $this->Ean_model->getRoomAmenities($this->data['lang_set']);
				...
				if (!empty ($search)) {
						...
						$this->data['eancheckin'] = trim($_GET['checkin']);
						$this->data['eancheckout'] = trim($_GET['checkout']);
						$this->data['child'] = (int) $_GET['child'];
						$this->data['minprice'] = $this->settings[0]->front_search_min_price;
				    $this->data['maxprice'] = $this->settings[0]->front_search_max_price;
...
// print_r($_POST);
								$city = trim(str_replace("-", ",", $_GET['city']));
...
if (!empty ($sprice)) {
		...
		public function hotel($hotelId, $customerSessionId,$checkin = null, $checkout = null) {
			$module = new stdClass;
			...
			if($isValidLang){
				...
				$checkinDate = $checkin;//trim($this->input->get('checkin'));
				$checkoutDate = $checkout;//trim($this->input->get('checkout'));
				...
				$arrayInfo['checkIn'] = str_replace("-","/",$checkinDate);
				$arrayInfo['checkOut'] = str_replace("-","/",$checkoutDate);
				...
				$related = $this->Ean_lib->getRelatedHotels($this->data['module']->location,$checkin, $checkout);
              ...
              /*  echo "<pre>";
				...
				$this->data['eancheckin'] = str_replace("-","/",$checkinDate);
				$this->data['eancheckout'] = str_replace("-","/",$checkoutDate);
		...
		/* For Payment Types Request and Response */
				...
				if($this->settings[0]->testing_mode == "1"){
					$pdata = (object) array("ai_first_name" => "Test Booking","ai_last_name" => "Test Booking",
					"accounts_email" => "test@hotmail.com","ai_mobile" => "1258463578","ai_country" => "US","ai_state" => "WA",
				"ai_city" => "Seattle","ai_postal_code" => "98004","ai_address_1" => "travelnow","cvv" => "123","cardtype" => "CA",
			"cardno" => "5401999999999999");
					$this->data['profile'] = array($pdata);
		...
		public function getResultInObjects($result,$checkin = null, $checkout = null,$adults = null,$agesApendUrl = null){
			...
			$response->totalCounts = $result['HotelListResponse']['HotelList']['@activePropertyCount'];
			...
			$ccheckin = str_replace("/","-",$checkin);
			$ccheckout = str_replace("/","-",$checkout);
				$slug = $this->data['baseUrl'].'hotel/'.$rs['hotelId'].'/'.$result['HotelListResponse']['customerSessionId'].'/'.$ccheckin."/".$ccheckout.$agesApendUrl;
		...
		public function getHotelDetailsObject($hotelid, $result){
			...
			$response->checkInInstructions =  html_entity_decode($hotelDetails['checkInInstructions']);
			$response->speicalCheckInInstructions =  html_entity_decode($hotelDetails['specialCheckInInstructions']);
