...
<?php ob_start(); mb_internal_encoding('utf-8');  $MESSAGE = "";
...
$USERNAME = "admin";
$HASHWORD = "c66020e7e9a5872be361bccb077225b8b4fa811890398ce8bd95859ac9937ca0";
...
$MAIN_WIDTH    = '900px'; //Width of main <div> defining page layout.          Can be px, pt, em, or %.  Assumes px otherwise.
...
$SESSION_NAME = 'OFCMS'; //Name of session cookie. Change if using multiple copies of concurrently.
...
?>
	...
	<form method="post" id="login_form" name="login_form" action="<?php echo $ONESCRIPT; ?>">
		...
		<input class="btn btn-primary btn-block btn-block" type="button"  class="button"   id="login" value="<?php echo hsc($_['Enter']) ?>">
	</form>
...
?>
	...
	<table class="index_T table table-responsive">
	<tr>
...
function Index_Page_buttons_top($file_count) {//********************************
	...
	if ($file_count > 0) {
		...
		echo '<button class="btn btn-warning" id=b3 tabindex='.$TABINDEX++.' type=button '.$onclick_d.'>'.$ICONS['delete'].hsc($_['Delete'])."</button\n>";
	}
	echo '</div>'."\n"; //end mcd_submit
	...
	echo "\n".'<form id=edit_form name=edit_form method=post action="'.$ONESCRIPT.$param1.$param2.$param3.'">'."\n";
		...
		if ( !in_array( mb_strtolower($ext), $ITYPES) ) { //If non-image...
			...
			elseif ( $text_editable && $too_large_to_view ) {
				echo '<p class="alert alert-success">'.$too_large_to_view_message.'</p>';
			} else {
...
function Countdown(count, End_Time, Timer_ID, Action){
	...
	if ((count == <?php echo $TO_WARNING ?>) && (Action != "")) { //Two minute warning...
		...
		var timeout_warning  = '<div class="alert alert-success"><b><?php echo hsc($_['session_warning']) ?></b> ';
	...
	if ( count < 1 ) {
		if ( Action == 'LOGOUT') {
			...
			$msg_box.innerHTML = '<div class=alert alert-success><b><?php echo hsc($_['session_expired']) ?></b></div>';
...
function Display_Messages($msg, take_focus) {//***********************
	...
	var $msg_div	 = '<div class=alert alert-success>' + $msg + '</div>';
	var $msg_box     = document.getElementById("message_box");
...
function events_down(event, capture_key) {
	...
	$pwun_msg_box.innerHTML = '<div class="alert alert-success"><b><?php echo hsc($_['Working']) ?></b>';
}
...
function pre_validate_pwun() {
	...
	if (($username.value == '') || ($pw.value == '') || ($new1.value == '') || ($new2.value == '')) {
		$pwun_msg_box.innerHTML = '<div class="alert alert-success"><b><?php echo hsc($_['change_pw_07']) ?></b>';
		return false;
	}
	//If new & confirm new values do not match...
	if (trim($new1.value) != trim($new2.value)) {
		$pwun_msg_box.innerHTML = '<div class="alert alert-success"><b><?php echo hsc($_['change_pw_04']) ?></b>';
		return false;
	}
	return true;
...
* { border : 0; outline: 0; margin: 0; padding: 0;
    ...
    font-size  : 14px; vertical-align: baseline; box-sizing: border-box; }
...
body { height: 100%; font-size: 14px; font-family: sans-serif; overflow-y: scroll; }
